<a href="<?php the_permalink() ?>" class="cursor-pointer block">
    <div class="hidden md:block image-box product-box overflow-hidden relative w-full">
        <div class="image absolute w-full h-full top-0 left-0 bg-gray-200 bg-cover bg-center bg-no-repeat" style="background-image:url(<?php the_post_thumbnail_url() ?>)"></div>
    ...
    <div class="block md:hidden p-3 cursor-pointer hover:bg-gray-300 w-full">
        <div class="flex items-center">
            <div class="w-1/3">
                <div class="h-24 w-full bg-gray-400 bg-cover bg-center bg-no-repeat bg-gray-200" style="background-image:url(<?= get_the_post_thumbnail_url($all_posts[$x]->ID) ?>)"></div>
            </div>
            <div class="w-2/3 pl-4">
                <h6 class="text-lg mb-2 leading-tight"><?php the_title() ?></h6>
                <div class="label inline-block"><span class="label-text inline-block leading-none"><?= get_the_category($all_posts[$x]->ID)[0]->name ?></span></div>
                <div class="label label-blue inline-block"><span class="label-text inline-block leading-none"><?= date('M j', strtotime($all_posts[$x]->post_date)) ?></span></div>
            </div>
        </div>
    </div>
</a>