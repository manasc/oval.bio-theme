...
<form role="search" method="get" class="woocommerce-product-search" action="<?= esc_url( home_url( '/'  ) ); ?>">
	...
	<input type="search" id="woocommerce-product-search-field" class="search-field" placeholder="<?= esc_attr_x( 'Search Products&hellip;', 'placeholder', 'woocommerce' ); ?>" value="<?= get_search_query(); ?>" name="s" title="<?= esc_attr_x( 'Search for:', 'label', 'woocommerce' ); ?>" />
