...
function Slide({ onClick, slidePosition, classNameFunc, currentSlide, slide, index }) {
    return (
        ...
        >
            <h1 className="text-5xl mb-2">{slide.title}</h1>
            <div>
                <p className="text-2xl opacity-50">{slide.content + (index === currentSlide ? "true" : "false")}</p>
            </div>
        </div>
    );
}
...
function TextSlider() {
    ...
    return (
        <section className="min-h-screen sm:overflow-hidden bg-gray-900 flex items-center">
            <div className="container">
                <div className="px-5 pb-10 items-center justify-center h-full w-full">
