				...
				<?php foreach ( wc_get_account_downloads_columns() as $column_id => $column_name ) : ?>
					<td class="<?= esc_attr( $column_id ); ?>" data-title="<?= esc_attr( $column_name ); ?>">
						...
						<?php elseif ( 'download-file' === $column_id ) : ?>
							<a href="<?= esc_url( get_permalink( $download['product_id'] ) ); ?>">
						...
						<?php elseif ( 'download-expires' === $column_id ) : ?>
							<?php if ( ! empty( $download['access_expires'] ) ) : ?>
								<time datetime="<?= date( 'Y-m-d', strtotime( $download['access_expires'] ) ); ?>" title="<?= esc_attr( strtotime( $download['access_expires'] ) ); ?>"><?= date_i18n( get_option( 'date_format' ), strtotime( $download['access_expires'] ) ); ?></time>
							<?php else : ?>
